...
// stream already exists with the same shape, it's a no-op. If the config
// differs (different subjects, retention, replicas), an error is returned
// so the operator can reconcile (issue #86 — workers and the control
// plane must agree on stream shape for the queue-group consumer to work).
func (p *NATSPublisher) EnsureStream(cfg StreamConfig) error {
	info, err := p.js.StreamInfo(cfg.Name)
	if err == nil {
		// Stream exists — verify config matches.
