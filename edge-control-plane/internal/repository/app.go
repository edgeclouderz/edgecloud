...
func (r *AppRepository) List(ctx context.Context, tenantID string) ([]domain.App, error) {
	var apps []domain.App
	query := `SELECT id, tenant_id, name, description, created_at FROM apps WHERE tenant_id = $1 ORDER BY name`
	err := r.db.SelectContext(ctx, &apps, query, tenantID)
	return apps, err
}
