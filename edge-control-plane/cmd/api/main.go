...
presets:[SwaggerUIBundle.presets.apis,SwaggerUIBundle.SwaggerUIStandalonePreset]})};
	...
	// Cluster autoscaler (issue #85). Disabled by default — operators
	// flip cfg.Autoscale.Enabled once the fleet has multiple workers
	// AND a real cloud-provider is wired in. The "noop" provider logs
	// intent without provisioning, so it's safe to enable for dev.
	//
	// The autoscaler reuses the publisher's NATS connection so it
	// shares heartbeats with WorkerService.SubscribeHeartbeats; both
	// consumers see every heartbeat (NATS fan-out). The autoscaler's
	// own goroutine pattern mirrors the heartbeat subscriber — both
	// exit cleanly on rootCtx cancellation.
	if cfg.Autoscale.Enabled {
		...
		go func() {
			if err := autoscaleSvc.Subscribe(rootCtx); err != nil {
