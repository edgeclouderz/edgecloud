...
async fn main() -> anyhow::Result<()> {
    ...
    let mut subscription = supervisor.nats.subscribe(&config.region).await?;
    ...
    // Main loop: process NATS messages
...
impl Supervisor {
    /// Perform graceful shutdown: stop all apps and publish a final heartbeat.
