...
impl RustAnalyzer {
    ...
    fn scoped_text(&self, source: &str, node: tree_sitter::Node) -> String {
        match node.kind() {
            "scoped_identifier" | "identifier" => node
...
"#;
        let matches = a.analyze(src);
        assert!(matches.is_empty(), "tokio::net is out of v1 scope, got {matches:?}");
    }
...
"#;
        ...
        sorted.sort();
        assert_eq!(lines, sorted, "matches should be sorted by line, got {lines:?}");
    }
}