...
- **`edge-migrate` placement.** Per `edge-migrate/docs/design.md` v0.2 (2026-06-18, the more authoritative doc for this tool), `edge-migrate` is a **standalone binary** (`cargo install edge-migrate`), not a subcommand of `edge-cli`. The older whitepaper §10.2 (2026-06-14) still describes it as an `edge migrate` subcommand — that description is **superseded by design.md**. The current code has a stub in `edge-cli/src/migrate/transformer.rs`; the real transform lives in `edge-migrate/edge-migrate-lib` and is invoked by the Go control plane as a subprocess via `edge-migrate --transform <path>`. Don't add new logic to the CLI stub.
